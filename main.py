...
from panda3d.core import LPoint3, LVector3, InputDevice
...
class AsteroidsDemo(ShowBase):
    ...
    def __init__(self):
        ...
        if devices:
            self.connect_gamepad(devices[0])

        # Accept device dis-/connection events
        self.accept("connect-device", self.connect_gamepad)
        self.accept("disconnect-device", self.disconnect_gamepad)

        # Other keys events set the appropriate value in our key dictionary
        self.accept("escape",               exit)
        self.accept("arrow_left",           self.set_key, ["turnLeft", 1])
        self.accept("arrow_left-up",        self.set_key, ["turnLeft", 0])
        self.accept("arrow_right",          self.set_key, ["turnRight", 1])
        ...
        self.accept("gamepad-face_a",       self.set_key, ["fire", 1])
        self.accept("gamepad-face_a-up",    self.set_key, ["fire", 0])
    ...
    def exit() -> None:
        """
        Exits the demo application
        """

        sys.exit()

    def connect_gamepad(self, device):
        """
    ...
    def disconnect_gamepad(self, device):
        """
    ...
    # to the given value.
    def set_key(self, key: str, val: object) -> None:
        """
        Sets the key value in the self.keys dictionary for use in game loop operations
        """

        self.keys[key] = val

    def set_velocity(self, obj: str, val: object) -> None:
        """
        Sets the velocity of a Panda3D object
        """
    ...
    def spawn_asteroids(self)-> None:
        ...
        for i in range(10):
            ...
            self.set_velocity(self.asteroids[i], v)

    def game_loop(self, task: object) -> None:
        """
        ...
        # update controller inputs to key mapping
        left_x = self.gamepad.findAxis(InputDevice.Axis.left_x)
        left_y = self.gamepad.findAxis(InputDevice.Axis.left_y)
        movement = Vec3(0)
        movement.x = left_x.value
        movement.y = left_y.value

        # Apply some deadzone, since the sticks don't center exactly at 0
        if movement.lengthSquared() >= 0.01:
            self.set_key("turnLeft",  movement.x <= 1.0)
            self.set_key("turnRight", movement.x >= 1.0)
            self.set_key("accel",     movement.y >= 1.0)
        else:
            self.set_key("turnLeft",  0)
            self.set_key("turnRight", 0)
            self.set_key("accel",     0)
        ...
        for ast in self.asteroids:
            ...
            if ((self.ship.getPos() - ast.getPos()).lengthSquared() <
                ...
                # Reset the velocity
                self.set_velocity(self.ship, LVector3(0, 0, 0))
                Sequence(Wait(2),          # Wait 2 seconds
                         ...
                         Func(self.spawn_asteroids)).start()  # Remake asteroids
                return Task.cont
    ...
    def updateShip(self, dt):
        ...
        if self.keys["accel"]:
            ...
            if newVel.lengthSquared() > MAX_VEL_SQ:
                newVel.normalize()
                newVel *= MAX_VEL
            self.set_velocity(self.ship, newVel)
